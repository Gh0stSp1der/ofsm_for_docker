...
#define SIG_TEST 44
#define SPARE_DISK "/dev/nvme0n1"
#define CHANGE_PERCENTAGE 90
//#define SPARE_DISK "/dev/sdd"   //HDD
//#define SPARE_DISK "/dev/sda" //SSD
...
//                printf("--- ProjID    block_count    UsedDisk(KB)    Con_total_size(MB) ---\n");
                ...
                if(CHANGE_PERCENTAGE < (float)(d.d_bcount*512)/(d.d_blk_hardlimit*512)*100){
                    for(j=0;j<MAX_CON;j++)
                    {
                        if(data[j].id == d.d_id)
                        {
